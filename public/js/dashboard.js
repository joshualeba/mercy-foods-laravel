document.addEventListener('DOMContentLoaded', function() {
    ...
    // --- INICIO: Delegación de Eventos para la Sección de Perfil (Versión Definitiva) ---
    const staticContentParent = document.querySelector('.main-content-inner');

    if (staticContentParent) {
        let originalProfileValues = {};
        let originalProfileImageSrc = '';

        // --- 1. LISTENER DE CLIC DELEGADO ---
        staticContentParent.addEventListener('click', function(e) {
            const target = e.target;
            const profileContainer = staticContentParent.querySelector('.profile-container');
            if (!profileContainer) return; // No estamos en la página de perfil

            // --- Lógica para el botón EDITAR (Usando .closest()) ---
            if (target.closest('#edit-button')) {
                // El clic fue en el botón de editar o en un hijo de él
                
                const editButton = profileContainer.querySelector('#edit-button');
                const saveButton = profileContainer.querySelector('#save-button');
                const cancelButton = profileContainer.querySelector('#cancel-button');
                const profileFieldset = profileContainer.querySelector('#profile-fieldset');
                const uploadButtonLabel = profileContainer.querySelector('.avatar-upload-button');
                const avatarPreview = profileContainer.querySelector('#avatar-preview');

                if (profileFieldset && saveButton && cancelButton && uploadButtonLabel && avatarPreview && editButton) {
                    originalProfileValues = {};
                    profileFieldset.querySelectorAll('input:not([type="file"]), textarea').forEach(input => {
                        originalProfileValues[input.name || input.id] = input.value;
                    });
                    originalProfileImageSrc = avatarPreview.src;

                    profileFieldset.disabled = false;
                    const nameInput = profileFieldset.querySelector('#name');
                    if (nameInput) {
                        nameInput.readOnly = true;
                        nameInput.style.backgroundColor = 'var(--main-bg)';
            ...
            // --- Lógica para el botón CANCELAR (Usando .closest()) ---
                ...
                const uploadButtonLabel = profileContainer.querySelector('.avatar-upload-button');
                const avatarPreview = profileContainer.querySelector('#avatar-preview');
                const profileImageInput = profileContainer.querySelector('#profile_image_main');

                if (profileFieldset && editButton && saveButton && uploadButtonLabel && avatarPreview && profileImageInput && cancelButton) {
                    ...
                    profileFieldset.querySelectorAll('input:not([type="file"]), textarea').forEach(input => {
                        ...
                        if (originalProfileValues.hasOwnProperty(key)) {
                            input.value = originalProfileValues[key];
                        }
                    });
                    avatarPreview.src = originalProfileImageSrc;
                    profileImageInput.value = '';
                }
            }

            // --- Lógica para CONTRASEÑA (Usando .closest()) ---
            else if (target.closest('#toggle-password-form')) {
                const togglePasswordBtn = profileContainer.querySelector('#toggle-password-form');
                const passwordSection = profileContainer.querySelector('#password-section');
                ...
                if (togglePasswordBtn && passwordSection) {
                    ...
                    togglePasswordBtn.textContent = isHidden ? 'Ocultar' : 'Cambiar Contraseña';
                }
            }

        }); // Fin del listener de clic delegado

        // --- 2. LISTENER DE CHANGE DELEGADO (Para la imagen) ---
        staticContentParent.addEventListener('change', function(e) {
             const target = e.target;
             const profileContainer = staticContentParent.querySelector('.profile-container');
             if (!profileContainer) return;
             const profileImageInput = profileContainer.querySelector('#profile_image_main');
             const avatarPreview = profileContainer.querySelector('#avatar-preview');
             if (target === profileImageInput && avatarPreview) {
        ...
        }); // Fin del listener de change delegado

    } // Fin del if (staticContentParent)
    // --- FIN: Delegación de Eventos para la Sección de Perfil ---
});